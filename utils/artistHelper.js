...
export function groupArtists(lives) {
  ...
  lives.forEach(live => {
    ...
    names.forEach((name, index) => {
      // 短すぎる名前や除外ワードを省く
      if (name.length > 1 && !['MC', '出演', 'ゲスト', '他', '企画'].includes(name)) {
        
        // かなデータがあればそれを使う。なければ元の名前をそのまま使う
        // (Python側で保存された順番に対応しています)
  ...
  Object.keys(groups).forEach(key => {
      groups[key].sort((a, b) => artistMap.get(a).localeCompare(artistMap.get(b), 'ja'));
  });
